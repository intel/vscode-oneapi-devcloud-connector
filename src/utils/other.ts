 ...
 */
 ...
 export function checkAndInstallExtension(extName: string): boolean {
	 ...
	 if (!remoteSSHext && !vscode.env.remoteName) {
		 ...
		 vscode.window.showInformationMessage('You must install the Remote-SSH extension by Microsoft to use the Intel Developer Cloud Connector for Intel oneAPI Toolkits', goToInstall)
			 .then((selection) => {
